...
// Re-export pure functions from scene-utils (canonical source for validators,
// constants, and shot grammar). Downstream Remotion code imports from here unchanged.
import {
  CAMERA_CONSTANTS as _CAMERA_CONSTANTS,
  resolveEntrancePrimitive as _resolveEntrancePrimitive,
} from '../../mcp/lib/scene-utils.js';

export {
  CAMERA_CONSTANTS,
  resolveEntrancePrimitive,
  validateScene,
  validateManifest,
  SHOT_SIZE_CSS,
  ANGLE_CSS,
...
// validateManifest and validateScene are now in scene-utils.js
...
// ── Camera Math ──────────────────────────────────────────────────────────────
// CAMERA_CONSTANTS and ENTRANCE_PRIMITIVES are now in scene-utils.js
// and re-exported at the top of this file.
...
export function resolveEntranceAnimation(layer, frame, fps, options = {}) {
  ...
  const primitive = _resolveEntrancePrimitive(primitiveId);
...
export function getCameraMotionValues(camera, progress, easingFn, clampBounds) {
  ...
  const intensity = camera.intensity ?? _CAMERA_CONSTANTS.DEFAULT_INTENSITY;
...
export function calculateOverscanDimensions(viewportW, viewportH, camera) {
  ...
  overscanX = Math.max(overscanX, _CAMERA_CONSTANTS.MIN_OVERSCAN);
  overscanY = Math.max(overscanY, _CAMERA_CONSTANTS.MIN_OVERSCAN);
...
// Shot grammar CSS constants + getShotGrammarCSS() are now in scene-utils.js
// and re-exported at the top of this file.
