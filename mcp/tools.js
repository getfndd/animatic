...
export function buildTools({
  ...
  return [
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          file_key: {
            type: 'string',
            description: 'Figma file key, or a full figma.com/file/... or /design/... URL.',
          },
          node_id: {
            type: 'string',
            description: 'Frame node id — API form `12:34` or URL form `12-34` both accepted.',
          },
          personality: {
            type: 'string',
    ...
    },
    {
      ...
      description:
        'Convert a raw Lottie animation (Bodymovin/lottie-web .json) into an Animatic v3 semantic scene (ANI-199). Parses the JSON without rendering — maps each visual layer (shape/text/image/precomp) to a semantic component with a placeholder HTML layer, infers descriptive types/roles from layer names + structure, extracts a brand palette from fill/stroke/gradient/solid/text colours, and emits a conservative staggered-entrance (`enter`) choreography with a reactive camera. Lottie\'s own motion is NOT translated — Animatic re-animates via the personality (vector fidelity is out of scope for v0). Accepts inline JSON content (string or object); `.lottie` ZIP containers are rejected with guidance. Stdio-only for v1 — the hostile-input bound isn\'t complete enough for the hosted edge yet (tracked in a follow-up issue). Returns { scene, report } — the report lists per-layer inferences, the palette, and advisories.',
      inputSchema: {
        type: 'object',
        properties: {
          lottie: {
    ...
    {
      name: 'record_render_feedback',
      description:
        'Record a human verdict on a rendered video (ANI-120). After someone watches a render, capture an overall up/down plus optional per-dimension notes (hook, narrative_arc, clarity, visual_hierarchy, motion_quality, brand_finish). Appends to the project\'s review/feedback.json with a full manifest snapshot + the render reference, so feedback is traceable to exactly what was judged. The conversational agent is the feedback surface — no UI. Pairs with recalibrate_scoring_weights.',
      inputSchema: {
        type: 'object',
        properties: {
          project: { type: 'string', description: 'Project slug or absolute path.' },
          verdict: { type: 'string', enum: ['up', 'down'], description: 'Overall verdict on the render.' },
    ...
    {
      name: 'recalibrate_scoring_weights',
      description:
        'Propose scoring-weight adjustments from accumulated render feedback across all projects (ANI-120). Explainable by construction: only human-named failure (down) notes move a weight, and every proposed adjustment cites the exact feedback entries that drove it. Returns a proposal (current vs proposed weights + a diff + evidence) — never mutates anything. Apply by passing the proposed weights to score_candidate_video. Fails closed with no proposal when evidence is thin.',
      inputSchema: {
        type: 'object',
        properties: {
          minEvidence: { type: 'number', description: 'Minimum number of per-dimension down-notes before a proposal is made. Default 3.' },
        },
      },
    },
    // ── Art direction ─────────────────────────────────────────────────────
    ...
    },
    {
      ...
      description:
        'Autonomous revision loop: scores per-scene, picks worst scenes, applies targeted revisions, re-scores, repeats until convergence or max rounds. Returns the best manifest with full revision history. With `frame_evidence: true`, whenever the JSON loop stalls it runs a rendered-frame pass (the ANI-178 hero-frame audit) and turns failing axes into bounded transforms the structural loop would miss; a frame fix can unlock further JSON rounds, so multiple bounded frame passes may run (capped per run, deduped). Cost-gated (renders only on stall, honors ANIMATIC_SKIP_REMOTION_RENDER, logs render count + ~seconds). Default behavior (flag off) is unchanged.',
      inputSchema: {
        type: 'object',
        properties: {
          ...
          frame_evidence: { type: 'boolean', description: 'Opt-in (default `false`). When `true`, a rendered hero-frame pass runs on each JSON-loop stall (bounded per run, deduped) to drive frame-evidence revisions. Expensive: renders one still per scene (degrades to metadata-only legibility when the toolchain/key is absent). Composition-geometry findings with no bounded lever are reported as advisories, never applied.' },
