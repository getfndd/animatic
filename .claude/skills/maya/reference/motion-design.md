...
> Full documentation: `docs/design-patterns/transitions.md`

Transitions communicate **spatial relationships**. Use the right pattern for the relationship:

| Pattern | Spatial Message | Use Case |
|---------|-----------------|----------|
| **Container Transform** | "This IS that" | Card → detail with shared element |
| **Shared Axis** | "This is NEXT TO that" | Tab switching, filters, pagination |
| **Drill** | "This is INSIDE that" | Hierarchical navigation (parent ↔ child) |
| **Layer** | "This is ON TOP of that" | Modal, slideout, dropdown, tooltip |
| **Fade** | "This REPLACES that" | Loading → content, skeleton → real data |

### Quick Decision Tree

```
Shared element persists? → Container Transform
Navigating hierarchy? → Drill (push right down, pull left up)
Switching peer content? → Shared Axis (horizontal)
Overlay appearing? → Layer (scale + fade)
Content swap in place? → Fade
```
...
|---------|----------|--------|
| Container Transform | normal (300ms) | out-quart |
| Shared Axis | quick (200ms) | out-quart |
| Drill | normal (300ms) | out-expo |
| Layer | quick (200ms) | out-quart |
| Fade | instant-quick (100-200ms) | out-quart |

### Refinement

Adjust duration and easing based on context:

**Duration adjustments:**
- Smaller elements → faster (use `instant` or `quick`)
- Larger travel distance → slightly longer
- Exit animations → 75% of entrance duration
- Mobile → cap at 200-300ms for navigation

**Easing adjustments:**
- `out-quart` → smooth, refined (default for most UI)
- `out-expo` → snappy, confident (navigation, decisive actions)
- `out-quint` → dramatic (hero animations, marketing moments)
- `in-quart` → exits (element leaving viewport)
- `in-out-quart` → toggles (back-and-forth states)
