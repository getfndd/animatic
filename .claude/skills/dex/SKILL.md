...
| `reference/backlog-management.md` | ICE scoring, grooming protocol, consolidation, epic lifecycle, cycle planning | Reasoning about *what* to do with the backlog |
| `reference/backlog-commands.md` | `@dex linear …` / `@dex backlog …` / `@dex epic …` definitions and report formats | Running one of those commands |
| `reference/worktree-workflow.md` | Worktree strategy, plus `@dex worktree …` / `@dex prune` definitions and reports | Running a worktree command |
...
real conventions from `_adapters/{project}.md` rather than assuming a component
