...
| Scope | `subdomain` | Global → omit; Surface/Component → use as subdomain |
...
| Rationale | `evidence` | |
| — | `tags: ["learning", "persona:maya"]` | Always include |
...
REFLEX scope (Global/Surface/Component) describes reach **within this project**.
...
1. **Is this a generalizable pattern or a project-specific rule?**
   - Pattern: "Never use ring-2 for focus states" → portable
   - Project rule: "Use token `bg-surface-raised`" → not portable
